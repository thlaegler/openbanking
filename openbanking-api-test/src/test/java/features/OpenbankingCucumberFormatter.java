 ...
 */
public class OpenbankingCucumberFormatter implements cucumber.api.Plugin, Reporter, Formatter {
  ...
  public OpenbankingCucumberFormatter(File file) {
  ...
  private static void setExtentHtmlReport(File file) {
    ...
    if (file == null || file.getPath().isEmpty()) {
      file = new File(OpenbankingCucumberProperties.INSTANCE.getReportPath());
    }
    if (!file.exists()) {
  ...
  /**
   * When running cucumber tests in parallel Klov reporter should be attached only once, in order to
   * avoid duplicate builds on klov server.
   */
      ...
      try {
        ...
        if (username != null && password != null) {
          MongoClientURI uri = new MongoClientURI("mongodb://" + username + ":" + password + "@"
              + hostname + ":" + port + "/?authSource=" + database);
