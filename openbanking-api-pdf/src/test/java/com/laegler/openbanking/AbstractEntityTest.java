...
/**
 * A utility class which allows for testing entity and transfer object classes. This is mainly for
 * code coverage since these types of objects are normally nothing more than getters and setters. If
 * any logic exists in the method, then the get method name should be sent in as an ignored field
 * and a custom test function should be written.
 *
...
public abstract class AbstractEntityTest<T> {
  ...
  /**
   * Creates an instance of {@link EntityTest} with the default ignore fields.
   */
  ...
  /**
   * Creates an instance of {@link EntityTest} with ignore fields and additional custom mappers.
   *
   ...
   */
  protected AbstractEntityTest(Map<Class<?>, Supplier<?>> customMappers, Set<String> ignoreFields) {
    this.ignoredGetFields = new HashSet<>();
    if (ignoreFields != null) {
      this.ignoredGetFields.addAll(ignoreFields);
    }
    this.ignoredGetFields.add("getClass");
    this.ignoredGetFields.add("isNew");
