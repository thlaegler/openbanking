...
@RestController
@Slf4j
public class CommercialCreditCardsApiController implements CommercialCreditCardsApi {
    ...
    @org.springframework.beans.factory.annotation.Autowired
    public CommercialCreditCardsApiController(ObjectMapper objectMapper, HttpServletRequest request) {
    ...
    public ResponseEntity<CommercialCreditCardResponse> commercialCreditCardsGet(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
		        ...
		        Object restResponse =
		            Transformation.transform(soapResponse, CommercialCreditCardResponse.class);
		        return new ResponseEntity<CommercialCreditCardResponse>((HttpStatus) restResponse);
                ...
                return new ResponseEntity<CommercialCreditCardResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<CommercialCreditCardResponse>(HttpStatus.NOT_IMPLEMENTED);
    }

    public ResponseEntity<Object> commercialCreditCardsHead(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
            	// TODO
