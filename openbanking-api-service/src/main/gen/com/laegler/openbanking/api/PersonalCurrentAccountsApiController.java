...
@RestController
@Slf4j
public class PersonalCurrentAccountsApiController implements PersonalCurrentAccountsApi {
    ...
    @org.springframework.beans.factory.annotation.Autowired
    public PersonalCurrentAccountsApiController(ObjectMapper objectMapper, HttpServletRequest request) {
    ...
    public ResponseEntity<PersonalCurrentAccountResponse> personalCurrentAccountsGet(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
		        ...
		        Object restResponse =
		            Transformation.transform(soapResponse, PersonalCurrentAccountResponse.class);
		        return new ResponseEntity<PersonalCurrentAccountResponse>((HttpStatus) restResponse);
                ...
                return new ResponseEntity<PersonalCurrentAccountResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<PersonalCurrentAccountResponse>(HttpStatus.NOT_IMPLEMENTED);
    }

    public ResponseEntity<Object> personalCurrentAccountsHead(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
            	// TODO
