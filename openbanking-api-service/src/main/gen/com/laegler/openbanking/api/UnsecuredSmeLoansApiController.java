...
@RestController
@Slf4j
public class UnsecuredSmeLoansApiController implements UnsecuredSmeLoansApi {
    ...
    @org.springframework.beans.factory.annotation.Autowired
    public UnsecuredSmeLoansApiController(ObjectMapper objectMapper, HttpServletRequest request) {
    ...
    public ResponseEntity<UnsecuredSMELendingResponse> unsecuredSmeLoansGet(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
		        ...
		        Object restResponse =
		            Transformation.transform(soapResponse, UnsecuredSMELendingResponse.class);
		        return new ResponseEntity<UnsecuredSMELendingResponse>((HttpStatus) restResponse);
                ...
                return new ResponseEntity<UnsecuredSMELendingResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<UnsecuredSMELendingResponse>(HttpStatus.NOT_IMPLEMENTED);
    }

    public ResponseEntity<Object> unsecuredSmeLoansHead(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
            	// TODO
