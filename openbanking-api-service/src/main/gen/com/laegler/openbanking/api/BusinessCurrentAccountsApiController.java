...
@RestController
@Slf4j
public class BusinessCurrentAccountsApiController implements BusinessCurrentAccountsApi {
    ...
    @org.springframework.beans.factory.annotation.Autowired
    public BusinessCurrentAccountsApiController(ObjectMapper objectMapper, HttpServletRequest request) {
    ...
    public ResponseEntity<BranchCurrentAccountResponse> businessCurrentAccountsGet(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
		        ...
		        Object restResponse =
		            Transformation.transform(soapResponse, BranchCurrentAccountResponse.class);
		        return new ResponseEntity<BranchCurrentAccountResponse>((HttpStatus) restResponse);
                ...
                return new ResponseEntity<BranchCurrentAccountResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<BranchCurrentAccountResponse>(HttpStatus.NOT_IMPLEMENTED);
    }

    public ResponseEntity<Object> businessCurrentAccountsHead(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
            	// TODO
