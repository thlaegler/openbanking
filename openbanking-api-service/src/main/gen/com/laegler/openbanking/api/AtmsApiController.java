    ...
    public ResponseEntity<ATMResponse> atmsGet(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
		        ...
		        Object restResponse =
		            Transformation.transform(soapResponse, ATMResponse.class);
		        return new ResponseEntity<ATMResponse>((HttpStatus) restResponse);
                ...
                return new ResponseEntity<ATMResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<ATMResponse>(HttpStatus.NOT_IMPLEMENTED);
    }

    public ResponseEntity<Object> atmsHead(@ApiParam(value = "Used for conditional request, to retrieve data only if modified since a given date" ) @RequestHeader(value="If-Modified-Since", required=false) String ifModifiedSince,@ApiParam(value = "Used for conditional request, to retrieve data only if the given Etag value does not match" ) @RequestHeader(value="If-None-Match", required=false) String ifNoneMatch) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
            	// TODO
